...
from owl.task_state import STOP_REASON_FINAL_ANSWER_RETURNED, TaskState


def test_run_store_creates_run_directory_and_state_file(tmp_path):
    store = RunStore(tmp_path / ".owl" / "runs")
...
def test_run_store_load_trace(tmp_path):
    store = RunStore(tmp_path / ".owl" / "runs")
    state = TaskState.create(run_id="run_005", task_id="task_005", user_request="Load trace.")
    store.start_run(state)

    store.append_trace(state, {"event": "run_started", "run_id": "run_005"})
    store.append_trace(state, {"event": "tool_executed", "tool_name": "read_file"})
    store.append_trace(state, {"event": "run_completed"})

    events = store.load_trace(state.run_id)
    assert len(events) == 3
    assert events[0]["event"] == "run_started"
    assert events[1]["tool_name"] == "read_file"


def test_run_store_load_trace_missing(tmp_path):
    store = RunStore(tmp_path / ".owl" / "runs")
    events = store.load_trace("nonexistent_run")
    assert events == []


def test_run_store_write_and_read_metrics(tmp_path):
    store = RunStore(tmp_path / ".owl" / "runs")
    state = TaskState.create(run_id="run_006", task_id="task_006", user_request="Metrics test.")
    store.start_run(state)

    metrics = {
        "outcome": {"task_success": True},
        "process": {"tool_call_count": 2},
        "efficiency": {"total_runtime_ms": 1000},
        "safety": {"policy_block_count": 0},
    }
    path = store.write_metrics(state, metrics)
    assert path.exists()

    loaded = json.loads(path.read_text(encoding="utf-8"))
    assert loaded["outcome"]["task_success"] is True
    assert loaded["process"]["tool_call_count"] == 2


def test_run_store_metrics_path(tmp_path):
    store = RunStore(tmp_path / ".owl" / "runs")
    state = TaskState.create(run_id="run_007", task_id="task_007", user_request="Path test.")
    assert str(store.metrics_path(state.run_id)).endswith("metrics.json")
