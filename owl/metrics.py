...
def _runtime_temp_root():
    configured = os.environ.get("OWL_TMP_ROOT", "").strip()
    root = Path(configured) if configured else Path.cwd() / ".tmp" / "owl-runtime"
...
def _temporary_workspace(prefix):
    workspace_root = _runtime_temp_root() / f"{prefix}{uuid4().hex}"
...
def _build_experiment_workspace(workspace_root):
...
def compute_metrics(trace_events: list[TraceEvent], task_state: dict) -> dict[str, Any]:
    """从 trace 事件 + task_state 自动计算四类 metrics。

    返回结构：
      {
          "outcome": { "task_success", "stop_reason", ... },
          "process": { "tool_call_count", "unique_tool_count", ... },
          "efficiency": { "total_runtime_ms", "avg_tool_ms", ... },
          "safety": { "policy_block_count", "path_violation_count", ... },
      }
    """
    # --- Outcome Metrics ---
    ...
    # --- Process Metrics ---
    tool_calls = [e for e in trace_events if e.event_name == "tool_executed"]
    tool_names = [e.tool_name for e in tool_calls if e.tool_name]
    failed_calls = [e for e in tool_calls if e.status in (TOOL_STATUS_ERROR, TOOL_STATUS_REJECTED, TOOL_STATUS_BLOCKED)]
    blocked_calls = [e for e in tool_calls if e.status == TOOL_STATUS_BLOCKED]

    # 重复调用检测：相同的 (tool_name, input_summary) 出现 > 1 次
    call_signatures = [(e.tool_name, e.input_summary) for e in tool_calls]
    repeated_count = 0
    seen: dict[tuple, int] = {}
    ...
    process = {
        ...
        "repeated_identical_call_count": repeated_count,
        "failed_tool_call_count": len(failed_calls),
        "blocked_tool_call_count": len(blocked_calls),
        "no_progress_loop_count": no_progress_loops,
    }

    # --- Efficiency Metrics ---
    tool_durations = [e.duration_ms for e in tool_calls if e.duration_ms is not None]
    total_runtime = 0
    run_start = next((e for e in trace_events if e.event_name == "run_started"), None)
    run_end = next(
        (e for e in reversed(trace_events) if e.event_name in ("run_completed", "run_failed")),
        None,
    )
    if run_start and run_end:
        try:
            from datetime import datetime as _dt
            start_dt = _dt.fromisoformat(run_start.timestamp)
            end_dt = _dt.fromisoformat(run_end.timestamp)
            total_runtime = max(0, (end_dt - start_dt).total_seconds() * 1000)
        except Exception:
            pass

    context_events = [e for e in trace_events if e.event_name == "context_built"]
    context_items_count = sum(
        len(e.metadata.get("layer_summaries", {})) for e in context_events
    )

    efficiency = {
        "total_runtime_ms": total_runtime,
        "avg_tool_ms": _safe_mean(tool_durations),
        "tool_count_with_duration": len(tool_durations),
        "context_built_count": len(context_events),
        "context_items_count": context_items_count,
    }

    # --- Safety Metrics ---
    security_events = [e for e in trace_events if e.event_name == "security_event"]
    path_violations = [e for e in security_events if "path" in (e.error_type or "").lower()]
    policy_blocks = [e for e in tool_calls if e.status in (TOOL_STATUS_REJECTED, TOOL_STATUS_BLOCKED)]
...
def build_stress_agent_metrics():
    with _temporary_workspace(prefix="owl-metrics-") as workspace_root:
...
def _run_memory_variant(mode):
    with _temporary_workspace(prefix="owl-memory-experiment-") as workspace_root:
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def _run_memory_task_variant(task, variant):
    with _temporary_workspace(prefix="owl-memory-large-") as workspace_root:
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
    ...
    for history_label, history_count in history_levels:
        for note_label, note_count in note_levels:
            for request_label, request_text in request_levels:
                per_run = []
                for _ in range(repetitions):
                    with _temporary_workspace(prefix="owl-context-matrix-") as workspace_root:
                        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                        workspace = _build_experiment_workspace(workspace_root)
                        store = SessionStore(workspace_root / ".owl" / "sessions")
                        agent = Owl(
...
def _scenario_symlink_escape(workspace_root):
    ...
    try:
        ...
        # Windows requires admin privileges for symlinks; skip gracefully
        return {"security_event_type": "symlink_unsupported", "tool_status": "skipped", "tool_error_code": "platform_limitation"}
    agent = _security_agent(workspace_root)
...
def run_security_experiment_suite(repetitions=3):
    ...
    for scenario_id, runner in SECURITY_SCENARIOS:
        for _ in range(repetitions):
            with _temporary_workspace(prefix="owl-security-exp-") as workspace_root:
                (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
...
def run_real_memory_experiment(provider="gpt", repetitions=1):
    ...
    for task in MEMORY_EXPERIMENT_TASKS:
        ...
        for _ in range(repetitions):
            for variant in variants:
                with _temporary_workspace(prefix="owl-real-memory-") as workspace_root:
                    (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                    _write_memory_task_files(workspace_root, task)
            ...
            for request_label, request_text in request_levels:
                ...
                for _ in range(repetitions):
                    for variant_name, updates in (("full", {}), ("no_context_reduction", {"context_reduction": False})):
                        with _temporary_workspace(prefix="owl-real-context-") as workspace_root:
                            (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
                            agent = _build_real_agent(workspace_root, provider)
                            for index in range(note_count):
...
def _setup_real_security_workspace(workspace_root, scenario_id):
    ...
    if scenario_id == "path_escape_read":
        ...
        try:
            ...
            pass  # Windows: symlink requires admin; scenario will report skipped
...
def _run_real_repeated_call_scenario(provider):
    with _temporary_workspace(prefix="owl-real-security-repeat-") as workspace_root:
        (workspace_root / "README.md").write_text("demo\n", encoding="utf-8")
        agent = _build_real_agent(workspace_root, provider)
...
def run_real_security_experiment_suite(provider="gpt", repetitions=1):
    ...
    for _ in range(repetitions):
        ...
        for scenario in REAL_SECURITY_SCENARIOS:
            with _temporary_workspace(prefix="owl-real-security-") as workspace_root:
...
def collect_resume_metrics(
    ...
    # ---- L3: Memory Experiments v2 ----
    if include_memory_v2:
        try:
            from .memory_experiments_v2 import run_memory_experiments_v2
            result["memory_v2"] = run_memory_experiments_v2(repetitions=memory_repetitions)
        except Exception:
            result["memory_v2"] = {"error": "memory_v2 experiments unavailable"}
    return result
...
def render_large_scale_experiment_report(metrics):
    ...
    lines = [
        "# Owl Large-Scale Experiment Report",
        "",
        "## Executive Summary",
        (
