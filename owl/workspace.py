...
IGNORED_PATH_NAMES = {".git", ".owl", "__pycache__", ".pytest_cache", ".ruff_cache", ".venv", "venv"}
...
class WorkspaceContext:
    def __init__(self, cwd, repo_root, branch, default_branch, status, recent_commits, project_docs, include_git_metadata=True):
        self.cwd = cwd
        ...
        self.include_git_metadata = bool(include_git_metadata)

    @classmethod
    def build(cls, cwd, repo_root_override=None, include_git_metadata=True):
        ...
        return cls(
            ...
            branch=(git(["branch", "--show-current"], "-") or "-") if include_git_metadata else "-",
            default_branch=(
            ...
            status=clip(git(["status", "--short"], "clean") or "clean", 1500) if include_git_metadata else "clean",
            recent_commits=[line for line in git(["log", "--oneline", "-5"]).splitlines() if line] if include_git_metadata else [],
            project_docs=docs,
    ...
    def fingerprint(self):
        ...
        payload = {
            ...
            "include_git_metadata": self.include_git_metadata,
        }
