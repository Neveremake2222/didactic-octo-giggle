...
这个模块负责把“用户怎么启动 owl”翻译成 runtime 能理解的对象：
...
def _spin(label="thinking"):
    """在后台线程中打印旋转动画，直到 stop_spin() 被调用。"""
...
def build_agent(args):
    ...
    - 输出：一个新的 `Owl`，或一个从旧 session 恢复出来的 `Owl`
    ...
    extra_names = os.environ.get("OWL_SECRET_ENV_NAMES", "")
    ...
    store = SessionStore(workspace.repo_root + "/.owl/sessions")
    model = _build_model_client(args)
    session_id = args.resume
    if session_id == "latest":
        session_id = store.latest()
    if session_id:
        return Owl.from_session(
            model_client=model,
            workspace=workspace,
            session_store=store,
